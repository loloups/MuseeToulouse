	...
	<body>
		...
		<div id="show-musee" class="content scaffold-show" role="main">
			...
			<ol class="property-list musee">
				...
				<g:if test="${museeInstance?.responsable}">
				<li class="fieldcontain">
					<span id="responsable-label" class="property-label"><g:message code="musee.responsable.label" default="Responsable" /></span>
					
						<span class="property-value" aria-labelledby="responsable-label"><g:link controller="gestionnaire" action="show" id="${museeInstance?.responsable?.id}">${museeInstance?.responsable?.encodeAsHTML()}</g:link></span>
